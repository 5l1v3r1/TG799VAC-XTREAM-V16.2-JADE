...
# BE CAREFUL WITH THIS SCRIPT // WUSEMAN
                ...
                if [[ ! -z $name ]]; then
                        ...
                        case $basename_process in
                                ...
                                *)
                                        ...
                                        if [[ $1 == "PID" ]]; then
                                                alive_processes="$(echo $i | sed 's|/proc/\([0-9]*\)/exe|\1|') 
$alive_processes";
                                        fi
                                        ;;
                        esac
        ...
        if [ -f /var/state/mmpbx ]; then
           last_mmpbx_state=`cat /var/state/mmpbx | grep "mmpbx.state" | tail -1`;
                if [ $last_mmpbx_state != "mmpbx.state='NA'" ]; then
                        ...
                        last_mmpbx_state=`cat /var/state/mmpbx | grep "mmpbx.state" | tail -1`; #last state is written in 
the last line of /var/state/mmpbx
...
case $# in
                ...
                if [[ $1 == "--all" ]]; then
                        ...
                        if [ "$OVERLAY_TYPE" = "jffs2" ] ; then
                                ...
                                device_nr=$(grep -E "(rootfs_data|userfs)" /proc/mtd | sed 
's/mtd\([0-9]\):.*\(rootfs_data\|userfs\).*/\1/')
                                mtd -r erase mtd${device_nr}
                        else
                                rm -rf /overlay/* ; sync ; reboot
                        fi
                elif [[ $1 == "--soft" ]]; then
                        ln -s $(mount | awk '/overlayfs/{print substr($1,length("overlayfs:")+1)}')     
/overlay/remove_due_to_rtfd
